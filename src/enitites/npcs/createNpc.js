...
const genericNpcAI = ({ currentEntityState, entities, scene }) => {
    ...
    if (currentEntityState.currentState === "recoil") {
        ...
        const knockbackForce =
            knockbackVector.getMagnitude() - knockbackVector.getMagnitude();
    }
    ...
    } else if (currentEntityState.currentState === "walking" && enemyTarget) {
    ...
    } else if (currentEntityState.currentState === "attacking" && enemyTarget) {
        nextTargetCoordinate = {
            x: enemyTarget.x,
            y: enemyTarget.y,
        };
...
export const useNpcAI = (key) => {
    return {
        GoblinArcher({ currentEntityState, entities, scene }) {
            return genericNpcAI({ currentEntityState, entities, scene });
        },
        BoulderMonster({ currentEntityState, entities, scene }) {
            return genericNpcAI({ currentEntityState, entities, scene });
        },
    }[key];
};
